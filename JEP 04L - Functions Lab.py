...
def fizzBuzz (num: int):
  assert type(num)==int, f"Expected actual to be of type int, but found {type(num)}"
...
#TODO
def testFizzBuzz (parameter: int, exp: int):        # Declare the function testFizzBuzz
  result = fizzBuzz(parameter) # Call fizzBizz() with the specified value
  assert type(result) == str, f"Expected actual to be of type str, but found {type(result)}."   # Assert that the result is of type str
  assert result == exp, f"""Expected "{exp}", "but found " {result}"."""   # Assert that the result matches the expected value
